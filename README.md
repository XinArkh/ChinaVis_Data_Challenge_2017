# 2017年第四届中国可视化与可视分析大会_数据可视分析挑战赛/挑战1（ChinaVis Data Challenge 2017）
...
2. 运行 `separateTime.py`，得到 24 个时间区间的数据 csv 文件，每个文件是 63 天中所有落在这一时间区间的数据（这一步耗时较久，优化思路在注释中给出，暂未实现），保存在`./time_zones`中
3. 运行 `csv2txt.py`，转换上一步得到的 csv 文件格式，保存在`loc_json`中
4. 将`loc_json`中的数据粘贴到`heatmap.html`中对应位置，打开网页即可得到该时段收到短信的位置热图